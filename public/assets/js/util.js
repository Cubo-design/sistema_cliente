const BASE_URL = "http://localhost/space_lashes/";
...
function uploadImg(input_file, img, input_path){
	...
	$.ajax({
		...
		beforeSend: function(){
			clearErrors();
			input_path.siblings(".help-blocks").html(loadingImg("Carregando imagem..."));
		},
		success: function(response){
			...
			if(response["status"]){
				...
				input_path.siblings(".help-blocks").html(response["error"]);
			}
		},
		error: function(){
			img.attr("src", src_before);
...
function enviarImg(input_file, img, input_path){
	...
	$.ajax({
		url: BASE_URL + "usuario/ajax_import_image",
		dataType: "json",
		cache: false,
		contentType: false,
		processData: false,
